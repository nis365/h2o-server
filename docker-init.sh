...
if [ -z "$node_name" ] ; then node_name=${HOSTNAME} ; fi
if [ -z "$port_num" ] ; then port_num=54321  ; fi
if [ -z "$mem_size" ] ; then mem_size='2g'  ; fi
java -Xmx${mem_size} -jar /opt/h2o-3.28.0.3/h2o.jar -name ${node_name}${gbm_model_version} -port ${port_num} -ice_root /tmp/ice_root